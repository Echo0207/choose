...
class Roulette:
    ...
    # 繪製轉盤選項
    def draw_options(self, pointer_angle=90):
        num_options = len(self.options)
    
        for idx, option in enumerate(self.options):
            start_angle = idx * (360 / num_options)
            stop_angle = (idx + 1) * (360 / num_options)
        ...
        # 用指針角度計算被選擇的選項索引
        selected_index = int(((360 - pointer_angle) / 360) * num_options)
    ...
    def run_roulette(self):
        ...
        target_pointer_angle = 360 - (selected_index * (360 / len(self.options)) + (360 / (2 * len(self.options))))
        ...
        while self.rotation_speed > 0.1:
            ...
            delta_angle = target_pointer_angle - current_pointer_angle
            if delta_angle > 180:
                delta_angle -= 360
